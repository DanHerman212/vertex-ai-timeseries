...
This repo includes end to end pipelines for time series forecasting on Vertex AI.  The pipeline trains two models to execute a prediction task.  The task is to predict minutes until the next train arrives to help passengers manage uncertainty in planning their commute.  The two models comprise of a simple RNN to provide point estimates, compared to a probabalistic model that uses quantile regression to predict specific points in the probability distribution of future arrivals, including the 10th, 50th, and 90th percentiles.
<br>
