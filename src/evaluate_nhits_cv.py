...
def evaluate_nhits_cv(model_dir, df_csv_path, metrics_output_path, html_output_path):
    ...
    if hasattr(model, 'trainer_kwargs'):
        ...
        print(f"Accelerator set to: {model.trainer_kwargs['accelerator']}", flush=True)
        
        # Remove callbacks (like EarlyStopping) that might fail during inference-only CV
